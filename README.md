...
### Вариант 1: DSL архитектура (рекомендуется) 🌟
```bash
# 1. Настройка
cp .env.example .env
...
### Вариант 2: Классическая архитектура
```bash
# 1. Настройка (аналогично)
cp .env.example .env

# 2. Установка зависимостей
pip install -r requirements.txt

# 3. Запуск классического бота
python run.py
# или
python bot.py
```
...
### Тестирование
1. Отправьте `/start` боту для регистрации
2. Используйте `/manager` для панели управления
3. Попробуйте `/onboarding` для полного онбординга (только DSL)
4. Команды отладки: `/scenario_info`, `/scenario_list` (только DSL)

**Готово!** Ваш бот готов к работе в любой архитектуре! 🎉

## 🎭 DSL возможности (новое!)

### Что дает DSL архитектура:
- 🎯 **Декларативные сценарии** - диалоги как данные
- 🔄 **Автоматическое восстановление** - продолжение после ошибок
- 🎭 **Композиции сценариев** - сложные workflow
- 📊 **Встроенная аналитика** - статистика выполнения
- 🔧 **Горячая перезагрузка** - обновление без перезапуска
- 🛡️ **Улучшенная надежность** - изоляция ошибок

### DSL команды:
- `/onboarding` - полный онбординг пользователя
